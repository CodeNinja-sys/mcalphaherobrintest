...
final class LinuxKeyboard {
    ...
    private void putKeyboardEvent(int keycode, byte state, int ch2, long nanos, boolean repeat) {
        this.tmp_event.clear();
        this.tmp_event.putInt(keycode).put(state).putInt(ch2).putLong(nanos).put(repeat ? (byte)1 : 0);
        this.tmp_event.flip();
    ...
    private void translateEvent(long event_ptr, int keycode, byte key_state, long nanos, boolean repeat) {
        ...
        if (num_chars > 0) {
            int ch2 = this.temp_translation_buffer[0];
            this.putKeyboardEvent(keycode, key_state, ch2, nanos, repeat);
            for (int i2 = 1; i2 < num_chars; ++i2) {
                ch2 = this.temp_translation_buffer[i2];
                this.putKeyboardEvent(0, (byte)0, ch2, nanos, repeat);
            }
        } else {
