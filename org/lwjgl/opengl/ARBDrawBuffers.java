...
public final class ARBDrawBuffers {
    ...
    public static void glDrawBuffersARB(IntBuffer buffers) {
        ...
        ARBDrawBuffers.nglDrawBuffersARB(buffers.remaining(), buffers, buffers.position(), function_pointer);
    }

    private static native void nglDrawBuffersARB(int var0, IntBuffer var1, int var2, long var3);

    public static void glDrawBuffersARB(int buffer) {
        ContextCapabilities caps = GLContext.getCapabilities();
        long function_pointer = caps.ARB_draw_buffers_glDrawBuffersARB_pointer;
        BufferChecks.checkFunctionAddress(function_pointer);
        ARBDrawBuffers.nglDrawBuffersARB(1, APIUtils.getBufferInt().put(0, buffer), 0, function_pointer);
    }
}
