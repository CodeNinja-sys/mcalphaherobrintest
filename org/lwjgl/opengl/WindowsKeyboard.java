...
final class WindowsKeyboard {
    ...
    private void putEvent(int keycode, byte state, int ch2, long millis, boolean repeat) {
        this.tmp_event.clear();
        this.tmp_event.putInt(keycode).put(state).putInt(ch2).putLong(millis * 1000000L).put(repeat ? (byte)1 : 0);
        this.tmp_event.flip();
