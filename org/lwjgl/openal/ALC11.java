...
public final class ALC11 {
    ...
    public static ALCdevice alcCaptureOpenDevice(String devicename, int frequency, int format, int buffersize) {
        long device_address = ALC11.nalcCaptureOpenDevice(devicename, frequency, format, buffersize);
        if (device_address != 0L) {
            ALCdevice device = new ALCdevice(device_address);
            HashMap hashMap = ALC10.devices;
    ...
    static native long nalcCaptureOpenDevice(String var0, int var1, int var2, int var3);
    ...
    public static void alcCaptureSamples(ALCdevice device, ByteBuffer buffer, int samples) {
        ALC11.nalcCaptureSamples(ALC10.getDevice(device), buffer, buffer.position(), samples);
    }

    static native void nalcCaptureSamples(long var0, ByteBuffer var2, int var3, int var4);

    static native void initNativeStubs() throws LWJGLException;

    static boolean initialize() {
        try {
            IntBuffer ib2 = BufferUtils.createIntBuffer(2);
            ALC10.alcGetInteger(AL.getDevice(), 4096, ib2);
            ib2.position(1);
            ALC10.alcGetInteger(AL.getDevice(), 4097, ib2);
            int major = ib2.get(0);
            int minor = ib2.get(1);
        ...
        catch (LWJGLException le2) {
            LWJGLUtil.log("failed to initialize ALC11: " + le2);
            return false;
        }
        return true;
    }
