...
public class Keyboard {
    ...
    private static void reset() {
        readBuffer.limit(0);
        for (int i2 = 0; i2 < keyDownBuffer.remaining(); ++i2) {
            keyDownBuffer.put(i2, (byte)0);
        }
    ...
    static {
        ...
        try {
            for (int i2 = 0; i2 < field.length; ++i2) {
                String name;
                if (!Modifier.isStatic(field[i2].getModifiers()) || !Modifier.isPublic(field[i2].getModifiers()) || !Modifier.isFinal(field[i2].getModifiers()) || !field[i2].getType().equals(Integer.TYPE) || !field[i2].getName().startsWith("KEY_")) continue;
                int key = field[i2].getInt(null);
                Keyboard.keyName[key] = name = field[i2].getName().substring(4);
