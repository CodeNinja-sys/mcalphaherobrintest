...
public class Cursor {
        ...
        switch (LWJGLUtil.getPlatform()) {
            ...
            case 3: {
                ...
                for (int i2 = 0; i2 < numImages; ++i2) {
                    ...
                    long delay = delays != null ? (long)delays.get(i2) : 0L;
                    long timeout = System.currentTimeMillis();
                    cursors[i2] = new CursorElement(handle, delay, timeout);
    ...
    private static void flipImages(int width, int height, int numImages, IntBuffer images, IntBuffer images_copy) {
        for (int i2 = 0; i2 < numImages; ++i2) {
            int start_index = i2 * width * height;
    ...
    private static void flipImage(int width, int height, int start_index, IntBuffer images, IntBuffer images_copy) {
        for (int y2 = 0; y2 < height >> 1; ++y2) {
            int index_y_1 = y2 * width + start_index;
            int index_y_2 = (height - y2 - 1) * width + start_index;
            for (int x2 = 0; x2 < width; ++x2) {
                int index1 = index_y_1 + x2;
                int index2 = index_y_2 + x2;
    ...
    public void destroy() {
        ...
        synchronized (object) {
            ...
            for (int i2 = 0; i2 < this.cursors.length; ++i2) {
                Mouse.getImplementation().destroyCursor(this.cursors[i2].cursorHandle);
            }
            this.destroyed = true;
        }
    }
