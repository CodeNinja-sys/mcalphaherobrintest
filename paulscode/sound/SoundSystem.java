...
public class SoundSystem {
    ...
    public void feedRawAudioData(String string, byte[] arrby) {
        this.CommandQueue(new CommandObject(22, string, arrby));
        this.commandThread.interrupt();
    }
    ...
    private void CommandFeedRawAudioData(String string, byte[] arrby) {
        if (this.soundLibrary != null) {
            this.soundLibrary.feedRawAudioData(string, arrby);
        } else {
    ...
    public static boolean libraryCompatible(Class class_) {
        ...
        boolean bl2 = SoundSystemConfig.libraryCompatible(class_);
        if (bl2) {
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    private static Class currentLibrary(boolean bl2, Class class_) {
        Object object = SoundSystemConfig.THREAD_SYNC;
        synchronized (object) {
            if (bl2) {
