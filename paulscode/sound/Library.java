...
public class Library {
    ...
    public int feedRawAudioData(String string, byte[] arrby) {
        ...
        return this.feedRawAudioData(source, arrby);
    }

    public int feedRawAudioData(Source source, byte[] arrby) {
        if (source == null) {
        ...
        if (!source.playing() || source.channel == null) {
            ...
            int n2 = source.feedRawAudioData(channel, arrby);
            channel.attachedSource = source;
            this.streamThread.watch(source);
            this.streamThread.interrupt();
            return n2;
        }
        return source.feedRawAudioData(source.channel, arrby);
    }
    ...
    private Channel getNextChannel(Source source) {
        ...
        if (source.toStream) {
            ...
            arrstring = this.streamingChannelSourceNames;
        } else {
            n3 = this.nextNormalChannel;
            list = this.normalChannels;
            arrstring = this.normalChannelSourceNames;
        }
        int n4 = list.size();
        for (n2 = 0; n2 < n4; ++n2) {
