            ...
            for (int j2 = 0; j2 < 16; ++j2) {
                ...
                for (int k1 = i2 - 1; k1 <= i2 + 1; ++k1) {
                    for (int i22 = j2 - 1; i22 <= j2 + 1; ++i22) {
                        int k2 = k1 + l2 & 0xF;
                        int i3 = i22 + i1 & 0xF;
                        f2 += this.field_1143_g[k2 + i3 * 16];
                    }
                }
                this.field_1142_h[i2 + j2 * 16] = f2 / 10.0f + (this.field_1141_i[(i2 + 0 & 0xF) + (j2 + 0 & 0xF) * 16] + this.field_1141_i[(i2 + 1 & 0xF) + (j2 + 0 & 0xF) * 16] + this.field_1141_i[(i2 + 1 & 0xF) + (j2 + 1 & 0xF) * 16] + this.field_1141_i[(i2 + 0 & 0xF) + (j2 + 1 & 0xF) * 16]) / 4.0f * 0.8f;
                int n2 = i2 + j2 * 16;
                this.field_1141_i[n2] = this.field_1141_i[n2] + this.field_1140_j[i2 + j2 * 16] * 0.01f;
                if (this.field_1141_i[i2 + j2 * 16] < 0.0f) {
                    this.field_1141_i[i2 + j2 * 16] = 0.0f;
                }
                int n3 = i2 + j2 * 16;
                this.field_1140_j[n3] = this.field_1140_j[n3] - 0.06f;
                if (!(Math.random() < 0.005)) continue;
                this.field_1140_j[i2 + j2 * 16] = 1.5f;
            }
        }
        float[] af2 = this.field_1142_h;
        this.field_1142_h = this.field_1143_g;
        this.field_1143_g = af2;
        for (int k2 = 0; k2 < 256; ++k2) {
            float f1 = this.field_1143_g[k2 - this.field_1139_k / 3 * 16 & 0xFF] * 2.0f;
            if (f1 > 1.0f) {
                f1 = 1.0f;
            }
            if (f1 < 0.0f) {
